...
execute "Authorizing SSH and ICMP traffic for default security group" do
  command "euca-authorize --region #{node["eucalyptus"]["dns"]["domain"]} -P icmp -t -1:-1 -s 0.0.0.0/0 default && euca-authorize --region #{node["eucalyptus"]["dns"]["domain"]} -P tcp -p 22 -s 0.0.0.0/0 default"
end
